...
class ImageMessageWriter(object):
    ...
    def encode(self, message):
        ...
        if len(messagesToEncode) + 32 > len(self.image.getdata()) * 8:
            raise Exception(
                "Error: Can't encode more than" + len(self.image.getdata()) * 8 - 32 + "bits in current image.")
        encodedPixels = []
        ...
        for index, (r, g, b, t) in enumerate(list(self.image.getdata())):
            if(index>=lastIndex):
                if (index % self.image0End.size[0] == 0):
                    lastIndex = index
                    break
        ...
        lenx = self.qrimage.size[0]
        leny = self.qrimage.size[1]
        qrdata = list(self.qrimage.getdata())

        ix = 0
        iy = 0

        print("Please wait for a long time.")

        for index, (r, g, b, t) in enumerate(list(self.image.getdata())):
            if(index>lastIndex):
                if(qrdata[ix*lenx+iy]==0x00):
                    encodedPixels.append((r, g, b, 0xfe))
                else:
                    encodedPixels.append((r, g, b, t))
                iy+=1
                if(iy==lenx):
...
class ImageMessageReader(object):
    ...
    def decode(self):
        pixels = list(self.image.getdata())
        bins = ''.join([str(int(r >> 1 << 1 != r)) + str(int(g >> 1 << 1 != g)) + str(int(b >> 1 << 1 != b)) + str(
...
if __name__ == '__main__':
    ...
    qrmessage = '姓名：阙开，电话号：10086'
    imgWriter.getqrImage(qrmessage)
